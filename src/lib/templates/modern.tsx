        ...
        <div className="bg-white text-slate-800 p-[25mm] font-sans leading-normal w-[210mm] mx-auto min-h-[297mm] flex flex-col shadow-none print:shadow-none">
            ...
            <header className="mb-10">
                <div className="flex items-end justify-between border-b-2 border-slate-900 pb-8">
                    <div>
                        <h1 className="text-5xl font-black text-slate-900 tracking-tight uppercase leading-none">{data.header.name}</h1>
                        <p className="text-slate-500 font-bold mt-4 uppercase tracking-[0.2em] text-[10pt]">{data.header.location}</p>
                    </div>
                    <div className="text-right text-[10pt] text-slate-500 space-y-1.5 font-bold">
                        <p>{data.header.phone}</p>
                        <p>{data.header.email}</p>
                        <div className="flex flex-wrap gap-5 justify-end mt-4 uppercase tracking-tighter">
            ...
            <div className="space-y-10 flex-1">
                ...
                {/* Education */}
                <section>
                    <h2 className="text-[10pt] font-black uppercase tracking-[0.3em] text-slate-400 mb-6 border-l-4 border-slate-100 pl-4">Academic History</h2>
                    <div className="space-y-6">
                        {data.education.map((edu, i) => (
                            <div key={i} className="flex justify-between items-baseline bg-slate-50/50 p-6 rounded-2xl border border-slate-100/60">
                                <div>
                ...
                {data.projects.length > 0 && (
                    <section>
                        <h2 className="text-[10pt] font-black uppercase tracking-[0.3em] text-slate-400 mb-6 border-l-4 border-slate-100 pl-4">Selected Projects</h2>
                        <div className="space-y-6">
                            {data.projects.map((proj, i) => (
                                <div key={i}>
                                    <div className="flex justify-between items-baseline mb-1">
                                        <h3 className="text-[12pt] font-extrabold text-slate-900 uppercase tracking-tight">{proj.name}</h3>
                                        {proj.link && <a href={proj.link} className="text-primary text-[10pt] font-black uppercase border-b-2 border-primary/20 hover:border-primary transition-all">View Project →</a>}
                                    </div>
                                    <p className="text-[10.5pt] text-slate-500 font-bold uppercase tracking-wide mb-3">{proj.techStack}</p>
                                    <ul className="space-y-2 text-[10.5pt] text-slate-600 font-medium leading-relaxed">
                ...
                {data.skills.categories && data.skills.categories.some(cat => cat.skills) && (
                    <section>
                        <h2 className="text-[10pt] font-black uppercase tracking-[0.3em] text-slate-400 mb-6 border-l-4 border-slate-100 pl-4">Technical Expertise</h2>
